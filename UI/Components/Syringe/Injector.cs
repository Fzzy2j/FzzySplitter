...
namespace Syringe
{
    ...
    public class Injector : IDisposable
    {
        ...
        public void DetectLibrary(string libPath)
        {
            ...
            if (modExists != null)
            {
                if (!injectedModules.ContainsKey(libName)) injectedModules.Add(libName, new InjectedModule(modExists));
                return;
            }
        }
        ...
        public void InjectLibrary(string libPath)
        {
            ...
            // iterate modules in target process to find if module is already injected
            ProcessModule modExists = null;
            foreach (ProcessModule mod in _process.Modules)
            {
                if (mod.ModuleName == libName)
            ...
            try
            {
                ...
                if(modFound == null)
                {
                    throw new Win32Exception(Marshal.GetLastWin32Error());//new Exception("Injected module could not be found within the target process!");
                }
        ...
        public void EjectLibrary(string libName)
        {
            ...
            // resources that need to be freed1
